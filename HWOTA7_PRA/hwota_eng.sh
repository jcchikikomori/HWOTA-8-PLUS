...
echo
echo -e "\e[1;33mReplacing the eRecovery runoff in TWRP, wait...\e[0m"
...
echo -e "\e[1;31mWrong select...\e[0m"
  ...
  RECOVERY_FILE=PRA_RECOVERY_NoCheck.img
...
if [ "$UPDATE_TYPE" = "2" ]; then
  ...
  do
    ...
    for oem in "${flist[@]}"
    do
      echo -e "\e[1;36m* $(format_str $idx.' '`echo $oem | $AWK -F "/" '{print $NF}' | $AWK -F "." '{print $1}'` 37)* \e[0m"
      idx=$(( idx + 1 ))
    done
    echo -e "\e[1;33m**************************************** \e[0m"
    echo -n "Select: "
